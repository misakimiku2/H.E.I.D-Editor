...
- **本轮平板审计顺手量出的档位不齐**(同日收口):菜单栏混档——Menu 与「新建」原 44×44、
  Markdown/CSV/JSON 视图三档切换原 44×40、状态栏上的编码/换行/还原按钮原高 44,
  而同排那八个图标钮是 48×48。不齐是**同一排内**的不齐,读起来像坏了。现全部抬到 48:
  横屏 14 个按钮最右缘 1262、竖屏 782,两档都 `scrollWidth == clientWidth`
  (菜单栏有 `flex-1` 余量 104dp,抬这 16dp 不吃紧);状态栏三条 48×48 / 66×48 / 86×48,
  与手机 StatusStrip 同款(48 高的条里放 48 高的触点)。跳行同时补了触屏开关(查找栏内 `#`
  按钮,与三个匹配选项同款开关态):平板实点跳到 Ln 9、手机实点跳到 Ln 12;手机第二行五个
  48dp 按钮之后计数标签仍完整可见(116dp,`scrollWidth == clientWidth` 未截断)。
- **停靠条上的按钮把输入法弄没了**(2026-09-22 用户反馈,同日修):点查找栏里的「展开替换」箭头,
  输入法先收起、之后不再自己弹回来,得再点一次输入框。根因是栏里的 `<button>` 抢走了输入框的焦点——
  Blink 在 tap 之后补发的那个 `mousedown` 的默认动作就是改焦点,WebView 随之收输入法且不会自己弹。
  修法两层:栏级 `onMouseDown` 里对**除 input 之外**的一切 `preventDefault()`(`click` 照常派发,
  禁用态的按钮也一并拦住——按到没反应的钮不该把键盘弄没);再配一个焦点交接,展开替换→替换框、
  开跳行→行号框、收起→交还查找框,否则焦点停在刚消失的那格上。预览查找条同样拦。
  桌面两处都不生效(`IS_TOUCH_PRIMARY` 门控),测试里桌面那例钉住「不拦 mousedown、不接管焦点」。
  实测(平板 1280×800 与手机 412×732,两端页面 script 哈希与 dist/index.html 逐字一致
  `index-D4L-_quC`,真 touch 事件、每步重量一次坐标):开栏→输入→展开替换→收起→区分大小写→
  下一个匹配→跳转到行,七步 `--heid-kb` 全程 368(手机 332)不掉,焦点依次落在
  查找 → 替换框 → 查找 → 查找 → 查找 → 行号框。
  探针教训:输入法开合会让停靠条整条上下移 336px,跨这一步复用的坐标必然打空——三次
  「点了没反应」全是这么来的,不是应用的锅。
- **仍是 44dp 档、本轮明确不动**:各弹窗(设置 / 关于 / Diff / 图片插入 / 确认框)、CSV 网格工具条、
  Markdown 表格结构条(60×44,有测试钉住)、SVG 画布工具条、取色器——这是一整层
  `pointer-coarse:min-h-[44px]` 的 v1.4 触屏档,抬到 48 要连那些测试与各自的排版一起重验,属另一批。
  标签条本身高 44(标题栏总高 48,再抬要动标签拖拽的占位几何)、标签关闭钮 28×28(见上,有意妥协)、
  行号槽与 `cm-colorDot`(见 ROADMAP 条目 41 尾)。
