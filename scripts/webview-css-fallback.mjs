 ...
 *
 * 有意未处理：ring-*。它的空 var 不在最终声明里，而是嵌在另一个自定义属性的值内部
 * （--tw-ring-shadow:var(--tw-ring-inset,) 0 0 0 …），要修得把改写伸进属性值内部，
 * 越过「只在规则层重排声明」这条边界。后果是老 WebView 上 ring 静默不画（实测
 * .ring-2 的 box-shadow 算成 none），不参与布局、不会位移。当前 src 只有 2 处用到：
 * 设置里主题卡片选中态（同处还有 border-[#A3B3FF] 兜着，认得出）、TabBar 的冲突橙点
 * （!isPhone 分支，手机上根本不渲染）。以后若要在触屏上加 ring 做唯一标识，得先补这条。
 *
 * 换算与判定都照实测定，不照规范想当然：
