...
27. **超大文件打开**：为 > 50MB 或百万行级文件引入分块 / 流式读取，只渲染可视区；语法高亮与查找按需关闭而非整体降级
28. **跨文件搜索**：文件夹范围内的文本检索（复用文件树已有的目录提供者；按需触发、不建常驻索引、不驻留后台进程），结果列表可跳转到文件行列
29. **查找上限解除**：当前 `searchCore.ts` 的 `MAX_SCAN_CHARS = 2_000_000` / `MAX_MATCHES = 5000` / `RESCAN_DOC_LIMIT` 在长文档上是硬边界
30. **小地图大文件适配**：固定画布高度 + 超阈值停用语法着色，先实测确认画布行数上限再实施（`CodeEditor.tsx` 未设置上限常量，属未验证结论）
31. **十六进制查看**：只读 hex 视图（二进制文件当前只能看乱码文本），与既有「二进制 · 只读」状态衔接
