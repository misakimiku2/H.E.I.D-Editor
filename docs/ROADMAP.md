...
> Markdown 大纲改为浮动毛玻璃小弹窗（与右键菜单同底，可收起为小徽标）；
> 修复粘贴图片预览失败（Windows 混合分隔符路径 fs 插件读取失败 → 路径归一跟随文档目录原生分隔符）
> 与页签文档高亮错位（块级图片 URL rewrite 改变了 data-md 偏移基准 + 跨块选区整块误包 →
> 移除 rewrite 收敛到 img 组件解析、行内操作按"块内选中文本"逐块应用）。
