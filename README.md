...
- **查找 / 替换 / 跳转到行**：`Ctrl+F` / `Ctrl+H` / `Ctrl+G`，支持正则、大小写、全词匹配与 `$1` 引用替换，全部匹配高亮 + 计数（自绘浮层，弹出在指针位置，桌面与手机一致体验）
- **Markdown 预览查找**：纯预览态 `Ctrl+F` 搜索渲染后的文本（不匹配源码标记），CSS Custom Highlight API 高亮全部匹配与当前项；分屏态搜索源码
- **编码支持**：UTF-8 / UTF-8 BOM / UTF-16 / GBK / GB18030 / Big5 / Shift_JIS 自动检测（Rust `encoding_rs`），状态栏可切换「以编码重新打开」或「转换编码并保存」；安卓端经 Kotlin 桥按编码写盘
- **换行符**：CRLF / LF / CR 打开时保留原样，状态栏一键转换，保存时还原
- **状态栏**：路径、语言、大小、行数、行列与选中字符数、换行符、编码
- **最近打开**：菜单二级列表（最多记忆 15 条）；空界面同样提供快捷入口
- **多标签页**：同时打开多个文件，脏状态标记，关闭前确认；会话恢复保留编码与换行符
- **设置面板**：字体 / 字号 / 行高 / Tab 宽度 / 缩进类型 / 自动换行 / 显示空白符 / 小地图 / 粘性滚动，修改即时生效
- **自动保存与崩溃恢复**：定时把有路径的脏文件落盘（默认关，可调间隔）；未保存内容始终写入本地草稿，异常退出后随会话恢复，「不保存」退出则明确丢弃
- **性能保护**：语言包按需加载（主包 gzip ≈ 546 KB）；超 200 万字符大文件自动降级（状态栏标注）；二进制文件只读预览
- **快捷键**：`Ctrl+S` 保存、`Ctrl+Shift+S` 另存为、`Ctrl+O` 打开、`Ctrl+N` 新建、`Ctrl+W` 关标签、`Ctrl+Tab` 切换标签、`Ctrl+Z` / `Ctrl+Y` 撤销重做、`Ctrl+F` 查找、`Ctrl+H` 替换、`Ctrl+G` 跳转到行（菜单内「键盘快捷键」有完整列表）
