...
- **顶栏**：标签数徽标（点开 TabSheet）+ 当前文件名（脏点）+ 视图切换（预览 / CSV 网格，按内容出现）+ 文件树侧栏开关 + ⋮ 溢出菜单。
  溢出菜单只放底栏没有的：新建 / 另存为 / 变更时间线 / 从网址导入 /（markdown）插入表格与插入图片 / 设置 / 快捷键 / 关于。
  打开文件与保存不在这里（底部工具栏已有），关标签也不在——只在 TabSheet 里做（v1.4.1 反馈）。
  隐藏自绘窗口按钮。
- **底部工具栏**（拇指区）：打开、撤销、保存、重做、查找。其上一条精简状态栏（语言 · 编码 · 换行符 · 行:列）。
  编辑/预览切换在顶栏，不占底栏（v1.4 反馈）。
- **设置**：手机端为整页（顶栏返回 + 底栏恢复默认/完成），平板与桌面仍是居中弹窗（v1.4.1 反馈：弹窗虽然够大，但设置项多、要滚，按页面走更顺手）。
